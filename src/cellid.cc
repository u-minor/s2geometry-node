...
Handle<Value> CellId::New(const Arguments& args) {
    ...
    if (info.Length() == 1) {
        ...
        if (Nan::New(Point::constructor)->HasInstance( fromObj)) {
            S2Point p = Nan::Nan::ObjectWrap::Unwrap<Point>(fromObj)->get();
            obj->this_ = S2CellId::FromPoint(p);
        } else if (Nan::New(LatLng::constructor)->HasInstance( fromObj)) {
            S2LatLng ll = Nan::Nan::ObjectWrap::Unwrap<LatLng>(fromObj)->get();
...
NAN_METHOD(CellId::FromToken) {
    ...
    CellId* obj = Nan::Nan::ObjectWrap::Unwrap<CellId>(info.This());
...
NAN_METHOD(CellId::Contains) {
    Nan::HandleScope scope;
    CellId* cellid = Nan::Nan::ObjectWrap::Unwrap<CellId>(info.This());
    S2CellId other = Nan::Nan::ObjectWrap::Unwrap<CellId>(info[0]->ToObject())->get();
